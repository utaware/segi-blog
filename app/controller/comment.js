 ...
 * @LastEditors: utaware
 * @LastEditTime: 2019-01-24 18:25:48
 */
...
class CommentController extends Controller {
  ...
  async index () {
    ...
    // 参数校验
    try {
      ctx.validate(app.validator.schema(['page', 'id']), {page: {no, size}, id: docs_id})
    } catch (err) {
  ...
  async create () {
    ...
    // 校验
    try {
      ctx.validate(app.validator.schema(['id', 'username', 'content']), {id: docs_id, username: replier_name, content})
    } catch (err) {
      return ctx.end(false, '参数校验未通过', err)
    }
    ...
    // 新增评论
    try {
      const find = await app.model.User.findOne({where: {username: replier_name}})
      const replier_id = find.user_id
      const floor = await app.model.Comment.currentfloor(docs_id)
      const result = await app.model.Comment.create({docs_id, user_id, user_name: username, replier_id, replier_name, floor, content})
